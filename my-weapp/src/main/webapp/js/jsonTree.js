...
(function ($) {
    $.fn.jsonTree = function (data, setting) {
        ...
        function createTreeItem(jsonData, $ul) {
            var $li = $("<li></li>");
            if (jsonData.name && jsonData.productCatalogId) {
                var $span = $("<span></span>").appendTo($li);
                var $icon = $("<i></i>").appendTo($span);
                $span.append(jsonData.name);
                $li.data("additional", jsonData.AdditionalData);
                $li.attr("value", jsonData.productCatalogId+','+jsonData.hasParts+","+jsonData.pcatalogId+","+jsonData.name);
                if ((jsonData.productCategory && jsonData.productCategory.length) || jsonData.HasSubItem) {

                    if (setting.lazyLoad && jsonData.HasSubItem) {
                        $li.addClass("parent_li");
                        $icon.addClass("icon-plus-sign");
                        var ajaxSendData = {};
                        ajaxSendData[setting.lazySendParameterName] = jsonData.productCatalogId;
                        $icon.click(function () {
                            $.ajax({
                    ...
                    if (jsonData.productCategory && jsonData.productCategory.length) {
                        var $innerUl = $("<ul></ul>");
                        $(jsonData.productCategory).each(function () {
        ...
        if (setting.forceCreate || !$this.data('treeView')) {
            ...
            $hasSubElements.addClass('parent_li').find(' > span').attr('name', 'Collapse this branch');
            $hasSubElements.find(' > span i').addClass(levelClass[1]);

            $('ul', $this).on('click', 'li > span', function (e) {
                var $clickedItem = $(this);
                getCategoryPart($(this).closest('li').attr('value'),1);
                ...
                if ($closestLi.hasClass("parent_li") && e.target && e.target.nodeName == "I") {
                    ...
                    if (children.is(":visible")) {
                        children.hide('fast');
                        $(this).attr('name', 'Expand this branch').find(' > i').addClass('icon-plus-sign').removeClass('icon-minus-sign');
                    } else {
                        children.show('fast');
                        $clickedItem.attr('name', 'Collapse this branch').find(' > i').addClass('icon-minus-sign').removeClass('icon-plus-sign');
                    }
                } else {
                    if (!$clickedItem.hasClass('selectedItem')) {
                        ...
                        if (setting.showCheckBox) {
                            ...
                            if (setting.selectedproductCatalogId > 0 && setting.selectedItemIsChecked == "True") {
            ...
            var $selectElement;
            if (setting.selectedproductCatalogId > 0) {
                $selectElement = $("li[value='" + setting.selectedproductCatalogId + "'] > span", $this);
            } else {
