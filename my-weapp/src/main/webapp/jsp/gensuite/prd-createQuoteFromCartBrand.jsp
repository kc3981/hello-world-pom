...
<body>
		...
		<form method="post">
			...
			<div class="container-fluid content-wrapper">
				...
				<div class="choose-brand">
					<p id="quoteBrandMessage" style="margin: 10px 0px 0px"></p>
				</div>
				...
				<div
					...
					<%-- <div class="horton col-lg-3 col-md-3 col-sm-4 col-xs-6 column-paddings brand-template">
	                           	<div class="brand-container contact-capacity select_brand" id="capacity_brand" data-brand="Capacity">
	                           		<c:choose>
	                                	<c:when test="${isFormCart}">
											<a href="../createquotefromcart/init?organizationId=${brands.organizationId}"><i>
												<img id="" src="${properties['docserver.url']}?media='+${brands.mediaId}+'&thumbnail=true&user='+userid+'&token='+authtoken+'" />	</i></a>
										</c:when>
										<c:otherwise>
											<a href="../directquote/init?organizationId=${brands.organizationId}"><i></i></a>
										</c:otherwise>
                                    </c:choose>
	...
	<script>
        $(document).ready(function(){
        	//Added by Krupa for Breadcrumbs Issue on 17-Nov-2015
        	var quoteJson=null;
	        ...
	        if(window.location.href.indexOf("directquotebrand") != -1 ){
	        	isdirectflag = true;
	        }
   		...
   		// End of Added by Krupa for Breadcrumbs Issue on 17-Nov-2015
         ...
         /* if (navigator.userAgent.indexOf('Safari') != -1 && navigator.userAgent.indexOf('Chrome') == -1){
        	...
        	for(var i=0;i<brandsJson.length;i++){
        		...
        		if(isFromCart=="true"){
        			...
        			if(brandsJson[i].canCreateQuote){
        				if(!brandsJson[i].createQuoteForNotMappedOrg){
        					string += '<div class="horton col-lg-4 col-md-4 col-sm-4 col-xs-12 column-paddings brand-template"> <div class="brand-container contact-capacity select_brand" id="capacity_brand" data-brand="Capacity"> <a href="directquoteinit?organizationId='+brandsJson[i].organizationId+'&isFromQuoteBrand=true&isDivisionMapped=false&breadcrumbsArray='+breadcrumbsPhrase+'"><i><img id="" src="${properties['docserver.url']}?media='+brandsJson[i].mediaId+'&thumbnail=true&user='+userId+'&token='+authToken+'" /></i></a> </div></div>'
        				}
        				else{
        					string += '<div class="horton col-lg-4 col-md-4 col-sm-4 col-xs-12 column-paddings brand-template"> <div class="brand-container contact-capacity select_brand" id="capacity_brand" data-brand="Capacity"> <a href="directquoteinit?organizationId='+brandsJson[i].organizationId+'&isFromQuoteBrand=true&isDivisionMapped=true&breadcrumbsArray='+breadcrumbsPhrase+'"><i><img id="" src="${properties['docserver.url']}?media='+brandsJson[i].mediaId+'&thumbnail=true&user='+userId+'&token='+authToken+'" /></i></a> </div></div>'
        				}
        				...
        				string += '<div class="horton col-lg-4 col-md-4 col-sm-4 col-xs-12 column-paddings brand-template not-active" style="pointer-events: none;"> <div class="brand-container contact-capacity select_brand" id="capacity_brand" data-brand="Capacity"> <a href="directquoteinit?organizationId='+brandsJson[i].organizationId+'&isFromQuoteBrand=true&isDivisionMapped=true"><i><img id="" src="${properties['docserver.url']}?media='+brandsJson[i].mediaId+'&thumbnail=true&user='+userId+'&token='+authToken+'" /></i></a> </div><p style="color:red;"><spring:message code="is.org.hasaccess" javaScriptEscape="true"/></p></div>'
        			}
        		}
        	}
        	...
        	// brand message
        	if(isFromCart=="true"){
        		var msg = "<spring:message code='label.brand.quote.msg1' javaScriptEscape='true'/>";
    			$('#quoteBrandMessage').text(msg);
    		}
    		else{
    			var msg = "<spring:message code='label.brand.quote.msg2' javaScriptEscape='true'/>";
    			$('#quoteBrandMessage').text(msg);
    		}
