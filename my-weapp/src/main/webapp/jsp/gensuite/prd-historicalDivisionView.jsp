			...
			if(!isdirectflag){
				if(isSearch || isQuoteOrder || isPromotions || isFromCartbc){
					if(isSearch) {
						...
						$(".breadcrumb > li:last-child()").after('<li><a href="'+$("#main-menu-list > li.active a").attr('href')+'"><spring:message code="quick.search.item" javaScriptEscape="true" />'+itemNumber+'</li>');
						$('#previousURL').val("");
          ...
          /*  if (navigator.userAgent.indexOf('Safari') != -1 && navigator.userAgent.indexOf('Chrome') == -1){
        	...
        	for(var i=0;i<brandsJson.length;i++){
        		...
        		if(isFromCart=="true"){
        			...
        			if(brandsJson[i].canPlaceOrderPOCC){
        				if(!brandsJson[i].createQuoteForNotMappedOrg){
        					string += '<div class="horton col-lg-4 col-md-4 col-sm-4 col-xs-12 column-paddings brand-template"> <div class="brand-container contact-capacity select_brand" id="capacity_brand" data-brand="Capacity"> <a href="gethistoricalorders?organizationId='+brandsJson[i].organizationId+'&isFromOrderBrand=true&partId=&quantity=&isDivisionMapped=false&chooseBrand=true&breadcrumbsArray='+breadcrumbsPhrase+'" onclick="maskingForPage()"><i><img id="" src="${properties['docserver.url']}?media='+brandsJson[i].mediaId+'&thumbnail=true&user='+userId+'&token='+authToken+'" /></i></a> </div></div>'
        				}
        				else{
            ...
            if(isFromCart=="true"){
            	...
            	if(brandsForCart=="[]"){
            		...
            		var msg = "<p class='text-left'><spring:message code='label.brand.vieworder' javaScriptEscape='true'/>";
