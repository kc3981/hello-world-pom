			...
			if(quoteStatus === 13 ){
				...
				//$('.content-wrapper').attr("readonly", "readonly");			
				 ...
				 if(quoteJson.paymentMethod.paymentMethodId==1 ||quoteJson.paymentMethod.paymentMethodId==2){
					...
					if(quoteStatus === 11 ||quoteStatus === 3){
						if(txnStatus){
					...
					if((quoteStatus === 11 ||quoteStatus === 3) && txnStatus==''&& isProcessOrderSuccessful==''){
						 $('.order-created-success').show();
						 $("#checkout-button").show();
					}else if(quoteStatus === 12){
						 $('.update-success').show();
						 $("#checkout-button").hide();
					}
				
				  }
				 if((isOrderNumberVisible=='false') && (quoteStatus === 11)){
